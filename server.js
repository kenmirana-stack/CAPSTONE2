  ...
  try {
    if (pool) {
      try {
        ...
        console.warn("Error ending old pool:", e && e.message)
      }
    }
  } finally {
...
async function addMissingColumnsPostgres() {
  try {
    ...
    const missingColumns = Object.entries(requiredColumns).filter(([column]) => !existingColumns.includes(column))
...
async function updateLocationSchemaPostgres() {
  try {
    ...
    const newColumns = ["contact", "operating_hours", "fuel_types", "services_offered", "description"]
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    const insertResult = await dbQuery("INSERT INTO users (name, email, password) VALUES ($1, $2, $3) RETURNING id", [
      name,
      email,
      hashedPassword,
    ])
...
app.post("/api/forgot-password", async (req, res) => {
  try {
    ...
    await dbQuery("UPDATE users SET reset_token = $1, reset_token_expiry = $2 WHERE email = $3", [
      resetToken,
      resetTokenExpiry,
      email,
    ])
...
app.post("/api/reset-password", async (req, res) => {
  try {
    ...
    const results = (
      await dbQuery("SELECT id, reset_token_expiry FROM users WHERE email = $1 AND reset_token = $2", [email, code])
    ).rows
    ...
    await dbQuery("UPDATE users SET password = $1, reset_token = NULL, reset_token_expiry = NULL WHERE email = $2", [
      hashedPassword,
      email,
    ])
...
app.get("/api/favorites", authenticateToken, async (req, res) => {
  try {
    const results = (await dbQuery("SELECT location_id FROM favorites WHERE user_id = $1", [req.user.id])).rows
...
app.post("/api/favorites", authenticateToken, async (req, res) => {
  try {
    ...
    await dbQuery("INSERT INTO favorites (user_id, location_id) VALUES ($1, $2)", [req.user.id, location_id])
...
app.delete("/api/favorites/:locationId", authenticateToken, async (req, res) => {
  try {
    const result = await dbQuery("DELETE FROM favorites WHERE user_id = $1 AND location_id = $2", [
      req.user.id,
...
app.post("/api/locations/:id/visit", optionalAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      await client.query(replacePlaceholders("INSERT INTO location_visits (user_id, location_id) VALUES (?, ?)"), [
        userId,
        req.params.id,
      ])
    ...
    if (error && (error.code === "ECONNRESET" || error.errno === "ECONNRESET")) {
      ...
      try {
        ...
        console.error("Failed to recreate DB pool:", e)
      }
    }
    res.status(500).json({ error: "Database error" })
  }
...
app.post("/api/locations/:id/view", optionalAuth, async (req, res) => {
  try {
    ...
    try {
      await client.query("BEGIN")
      await client.query(replacePlaceholders("INSERT INTO location_views (user_id, location_id) VALUES (?, ?)"), [
        userId,
        req.params.id,
      ])
...
app.get("/api/locations/:id/reviews", async (req, res) => {
  try {
    ...
    const averageRating = avgResult[0].average_rating ? Number.parseFloat(avgResult[0].average_rating).toFixed(1) : 0
...
app.post("/api/locations/:id/reviews", authenticateToken, async (req, res) => {
  try {
    ...
    const existingReview = (
      await dbQuery("SELECT id FROM reviews WHERE user_id = $1 AND location_id = $2", [userId, locationId])
    ).rows

    if (existingReview.length > 0) {
      await dbQuery("UPDATE reviews SET rating = $1, comment = $2 WHERE user_id = $3 AND location_id = $4", [
        rating,
        comment,
        userId,
        locationId,
    ...
    await dbQuery("INSERT INTO reviews (user_id, location_id, rating, comment) VALUES ($1, $2, $3, $4)", [
      userId,
      locationId,
      rating,
      comment,
...
app.get("/api/admin/stats", authenticateAdmin, async (req, res) => {
  try {
    ...
    const stationCount = (
      await dbQuery("SELECT COUNT(*) as count FROM locations WHERE type = $1 AND is_archived = FALSE", ["gasoline"])
    ).rows
    const shopCount = (
      await dbQuery("SELECT COUNT(*) as count FROM locations WHERE type = $1 AND is_archived = FALSE", ["repair"])
    ).rows
  ...
  try {
    ...
    res.json({
      location: {
        ...
        lng: Number.parseFloat(locationData.lng),
...
app.post("/api/admin/locations", authenticateAdmin, upload.single("image"), async (req, res) => {
  try {
    ...
    const { name, address, lat, lng, type, contact, operating_hours, fuel_types, services_offered, description } =
      req.body
...
async function unarchiveLocationHandler(req, res) {
  try {
    const locationId = req.params.id
    const result = await dbQuery("UPDATE locations SET is_archived = FALSE, archived_at = NULL WHERE id = $1", [
      locationId,
    ])
...
app.post("/api/admin/locations/:id/image", authenticateAdmin, upload.single("image"), async (req, res) => {
  try {
    ...
    const result = await dbQuery("UPDATE locations SET image = $1 WHERE id = $2", [imageUrl, locationId])
...
app.delete("/api/admin/locations/:id", authenticateAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      const result = await client.query(replacePlaceholders("DELETE FROM locations WHERE id = ? RETURNING id"), [
        locationId,
      ])
